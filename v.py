...
# Modern CSS with Glassmorphism + Dark Theme + Fixed File Uploader
st.markdown("""
<style>
    ...
    * {
    ...
    /* Dark Gradient Background - Deep Ocean */
    .stApp {
        background: linear-gradient(135deg, #203a43 50%, #ADD8E6 10%, #2c5364 90%); 
    }
    ...
    [data-testid="stFileUploader"] > section:first-child {
    ...
    [data-testid="stFileUploader"] > section:first-child:hover {
        border-color: rgba(0, 245, 255, 0.6) !important;
        background: rgba(255, 255, 255, 0.08) !important;
        box-shadow: 0 0 20px rgba(0, 245, 255, 0.2) !important;
    }

    [data-testid="stFileUploader"] > section:first-child > div > button {
        background: linear-gradient(135deg, #667eea 0%, #764ba2 100%) !important;
        border: none !important;
    ...
    [data-testid="stFileUploader"] > section:first-child > div > button:hover {
    ...
    /* Hide any duplicate text overlays */
...
with st.sidebar:
    ...
    <div style="text-align: center; padding: 30px 20px; background: linear-gradient(135deg, rgba(102,126,234,0.3) 0%, rgba(118,75,162,0.3) 100%); 
        ...
        <p style="color: rgba(255,255,255,0.6); margin-top: 8px; font-size: 0.85em;">Rovix Technology</p>
    </div>
    """, unsafe_allow_html=True)
...
with col_time:
    ...
    <div class="glass-card" style="text-align: center; padding: 16px;">
        <p style="color: rgba(255,255,255,0.6); font-size: 0.85em; margin: 0;">📅 {current_date}</p>
        <p style="color: #00f5ff; font-size: 1.2em; margin: 8px 0 0 0; font-weight: 600;">🕐 {current_time}</p>
    </div>
    """, unsafe_allow_html=True)
...
# ================================================================================
# DATA INPUT - MODERN CARDS (FIXED FILE UPLOADER)
# ================================================================================
...
with input_col1:
    ...
    # File uploader - NO glass-card wrapper to prevent double text
...
if len(df) > 0:
    ...
    for col, (label, value, color, key) in zip(kpi_cols, kpi_data):
        with col:
            ...
            try:
                val_num = int(value.split()[0]) if value.split()[0].replace('-','').isdigit() else 75
                progress_width = min(100, val_num)
            except:
                progress_width = 75
            ...
            <div class="kpi-card">
                ...
                <div class="progress-modern">
                    <div class="progress-modern-fill" style="width: {progress_width}%"></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if selected_division:
            ...
            fig_div_issue = px.bar(div_issues, y='Issue', x='Count',
    ...
    with tab5:
        ...
        if selected_machine:
            ...
            fig_m = px.bar(machine_issues, y='Issue', x='Count',
    ...
    with tab6:
        ...
        with f5:
            time_filter = st.selectbox(
                "⏱️ Filter by Time",
                ["All", "Above 30 mins", "Below 30 mins", "Above 60 mins", "Below 15 mins", "Custom Range"],
                index=0
            )
            ...
            if time_filter == "Custom Range":
                c1, c2 = st.columns(2)
                with c1:
                    custom_min = st.number_input("Min (mins)", min_value=0, value=0, step=5)
                with c2:
        ...
        if time_filter == "Above 30 mins":
            filtered = filtered[filtered[time_col] > 30]
        elif time_filter == "Below 30 mins":
            filtered = filtered[filtered[time_col] < 30]
        elif time_filter == "Above 60 mins":
            filtered = filtered[filtered[time_col] > 60]
        elif time_filter == "Below 15 mins":
            filtered = filtered[filtered[time_col] < 15]
